...
//!
//! KEY FINDING (the whole reason this module exists):
//!
...
//!
//! Python sidestepped this by emitting `mpris:trackid` as Variant("s") (a plain
//! string); playerctl tolerates it, but it violates the spec ("o"). For the
//! Rust port we instead ENCODE the YouTube ID into a guaranteed-valid object
//! path. `-` -> `_2d` (its hex code, prefixed to disambiguate from a literal
//! `_`), and we escape `_` itself as `_5f` so the mapping stays reversible.
//! This yields a spec-compliant `o` value AND round-trips back to the YT ID.
...
pub fn youtube_trackid(video_id: &str) -> TrackId {
    ...
    // This `try_from` is exactly the call that would PANIC/Err on a raw YT id
    // with a `-`. Because we encoded, it is now infallible in practice; we
    // still handle the error rather than unwrap to keep the contract honest.
